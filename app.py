...
r"""
PDFページ欠落チェック（スキャンPDF向け） v1.2.16

v1.2.16 変更点
- v1.2.15で発生した TypeError: 'NoneType' object is not subscriptable を修正
  （ログ出力時にホワイトリストがNoneの場合のガード処理を追加）

v1.2.15 変更点
- ローマ数字（Roman numerals）の認識精度向上（厳格モード追加）
v1.2.14 変更点
- 開始位置の自動補正（表紙ノイズ対策）
...
def ocr_extract_value(
    ...
    whitelist_mixed = "0123456789０１２３４５６７８９IVXLCDMivxlcdm()[]-–— "
    whitelist_roman = "IVXLCDMivxlcdm" # v1.2.15: Strict Roman whitelist

    # whitelists to try
    # 1. Mixed
    # 2. Strict Roman (if enabled) -> This forces OCR to pick letters even if they look like digits
    whitelist_list: List[Optional[str]] = [whitelist_mixed, None]
    if roman_enabled:
        whitelist_list.insert(1, whitelist_roman)
                ...
                for thr in thr_list:
                    ...
                    for psm in psm_list:
                        ...
                        for wl in whitelist_list:
                            ...
                            # v1.2.16 fix: handle wl=None in f-string
                            safe_wl = (wl[:5] + "...") if wl else "off"

                            if v is not None:
                                cand = OCRResult("arabic", v, str(v), joined, rot, True,
                                                 f"data psm={psm} thr={thr_use} inv={inv} bin={binarize} wl={safe_wl}")
                                maybe_update(cand)
                                found_any = True
                                break
                        ...
                        for wl in whitelist_list:
                            ...
                            if v2 is not None:
                                cand2 = OCRResult("arabic", v2, str(v2), raw_s, rot, True,
                                                  f"str psm={psm} thr={thr_use} inv={inv} bin={binarize} wl={safe_wl}")
                                maybe_update(cand2)
                                break
...
class App(tk.Tk):
    ...
    def _worker_analyze(self):
        try:
            ...
            prev_type = None
            prev_val = None
            prev_pdf_page = None # v1.2.13: 最後にprev_valを更新したPDFページ
            ...
            for p in range(1, total + 1):
                ...
                # --- v1.2.14: 開始位置の自動補正（表紙ノイズ対策） ---
                # 表紙等で「43」と誤検知した直後、物理7ページ以内に「1〜5」が出たら
                # 逆転NG判定を覆して「ここが開始位置」とみなす
                if sev == "NG" and p <= 7:
                    if prev_val is not None and dint is not None:
                        # 大きな値から小さな値へのリセットか？
                        if prev_val > dint and dint <= 5:
                            sev = "OK"
                            reason = f"開始位置補正（前値={prev_val}を除外）"

                # --- v1.2.13: 暴走・同期ズレ防止ロジック ---
                if ocr.ok:
                    ...
                    if prev_val is None:
                        ...
                        if sev == "OK":
                            ...
                            # 1. 物理ページ差分による再同期チェック (NG後の復帰用)
                            # NGが続いてprev_valが古くても、PDFページ数と番号の増分が一致すれば採用する
                            page_diff = p - prev_pdf_page if prev_pdf_page else 0
                            expected_val = prev_val + page_diff
                            
                            # 多少のズレ(±2)は許容してマッチング（OCR誤読や開始位置ズレの吸収）
                            if abs(dint - expected_val) <= 2:
                                is_valid_update = True
                            
                            # 2. 従来のマージンチェック（通常運用）
                            # 物理マッチしなくても、前回の番号から極端に離れていなければ採用
                            else:
                                gap = dint - prev_val
                                if -1 <= gap <= 50:
                                    is_valid_update = True
